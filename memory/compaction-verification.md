...
3. **Verify memory preservation:**
   - Check `memory/YYYY-MM-DD.md` for recent entries
...
- **Verification**: Check `memory/` for preserved context
...
3. **Alchemical insights** (from `memory/insights.md`)
4. **Backlog items** (from `memory/nightly-backlog.md`)
