...
type Model = "flux" | "nano-banana" | "nano-banana-pro" | "gpt-image-1" | "fal-flux" | "fal-flux-pro";
...
function parseArgs(argv: string[]): CLIArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (key) {
      case "model":
        if (value !== "flux" && value !== "nano-banana" && value !== "nano-banana-pro" && value !== "gpt-image-1" && value !== "fal-flux" && value !== "fal-flux-pro") {
          throw new CLIError(`Invalid model: ${value}. Must be: flux, nano-banana, nano-banana-pro, gpt-image-1, fal-flux, or fal-flux-pro`);
        }
        parsed.model = value;
